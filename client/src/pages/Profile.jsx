...
/>
        <p className='text-sm self-center'>
          {fileUploadError ?
            (<span className='text-red-700'>Error uploading Image (image must be less than 2 mb)
            </span>) :
          filePerc > 0 && filePerc < 100?(
             <span className='text-green-700'>{`Uploading ${filePerc}%`}
             </span>)
             :
             filePerc === 100?(
        ...
        <Link className='bg-green-700 text-white text-center p-3 rounded-lg uppercase hover:opacity-95' to={"/create-listing"}>Create Listing</Link>     
      </form>
      ...
      <p>{showListingsError ? 'Error loading listings' : ''}</p>
      {userListings && 
